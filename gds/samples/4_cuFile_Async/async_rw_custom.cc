 ...
 * with custom stream, where the stream is created with
 ...
 *
 * ./async_rw_custom <testWriteReadFile> <testWriteFile> <gpuid>
 *
 * | Output |
 * Using custom stream for cuFileAsync I/O
...
int main(int argc, char *argv[]) {
	...
	std::cout << "Using custom stream for cuFileAsync I/O" << std::endl;
	...
	wfd = open(testWriteFile.c_str(), O_CREAT | O_RDWR | O_DIRECT, 0664);
	if (wfd < 0) {
	...
	// Create custom stream for I/O and zero out in the case of holes
...
// Cleanup labels
stream_destroy:
	if (io_stream) check_cudaruntimecall(cudaStreamDestroy(io_stream));
