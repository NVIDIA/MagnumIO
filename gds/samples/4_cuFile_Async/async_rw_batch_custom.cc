 ...
 * the async APIs are used in a batch mode. In this case, we create custom
 * streams with cudaStreamCreateWithFlags.
 ...
 *
 * ./async_rw_batch_custom <testWriteReadFile> <testWriteFile> <gpuid>
 *
 * | Output |
 * Using cuFileAsync APIs in a batch mode with custom streams
...
int main(int argc, char *argv[]) {
	...
	std::cout << "Using cuFileAsync APIs in a batch mode with custom streams" << std::endl;
...
deregister_bufferhandles:
	for (size_t i = 0; i < registered_buffers; i++) {
		...
		if (io_stream[i] != nullptr) check_cudaruntimecall(cudaStreamDestroy(io_stream[i]));
	}
