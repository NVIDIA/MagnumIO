 ...
 */
 ...
 /*
 * This sample shows large number of threads, each with separate 
 ...
 * ...
 * Successful read of 68719476736 bytes on thread 40 from fd 113 to GPU id 0 with registered buffers
 * Successful read of 68719476736 bytes on thread 60 from fd 133 to GPU id 0 with registered buffers
 * ...
 */
 ...
 */
static void thread_fn(std::promise<int>&& funcRet, int tid, int gpuid, int fd, bool isreg) {
	...
	// Register the 64 GPU buffers with cuFile if registered flag is set otherwise just allocate the memory
	...
	// Read the data in chunks into the GPU buffers
	for (size_t i = 0; i < NUM_GPU_BUFFERS && success; i++) {
		offset = 0;
		for (int j = 0; j < nr_ios; j++) {
...
deregister_bufferhandles:
	// Deregister the GPU buffers if they were registered
	...
	if (success) {
		std::cout << "Successful read of " << totalBytesRead << " bytes on thread "
			<< tid << " from fd " << fd << " to GPU id " << gpuid  << " with " 
			<< (isreg ? "registered" : "unregistered") << " buffers" << std::endl;
		funcRet.set_value(0);
	} else {
		funcRet.set_value(-1);
	}
...
int main(int argc, char *argv[]) {
	...
	std::array<bool, NUM_THREADS> isRegistered;
	...
	for (size_t i = 0; i < NUM_THREADS; i++) {
		std::string file = argv[(i % 2) + 1];
		fds[i] = open(file.c_str(), O_RDONLY | O_DIRECT);
		if (fds[i] < 0) {
			std::cerr << "Error: Unable to open file " << argv[(i % 2) + 1] << " for fd " << fds[i] << std::endl;
			// Close all file descriptors that were opened prior to this failure
			closeFds(fds);
			return EXIT_FAILURE;
		}
		isRegistered[i] = (i % 2 == 0);
	}
	...
	for (size_t tid = 0; tid < NUM_THREADS; tid++) {
		...
		threadPool[tid] = std::thread(thread_fn, std::move(funcRet), tid, gpuid, fds[tid], isRegistered[tid]);
	}
