 ...
 * cuMemMap-ed allocations. cuFile pre-emptively registers the expected resized
 * device memory and writes the data to the files. Thus no re-registration is
 * needed. It then proceeds to clear the device memory and read the data back
 * from the files. Vector addition is then performed on the GPU and the result
 * is verified. Through the usage of cuMemMap-ed allocations, the user can
 * specify the physical properties of their memory
 ...
 *
 * ./memmap_preregister <testfileA> <testfileB> 
 * 
 * | Output |
 ...
 * h_A(0xfb9bf2200010), h_B(0xfb9be4400010), h_C(0xfb9bd6600010), elements: 57671680
 * ...
 * Result = PASS
 * Cleaning up
 * Freeing d_A
...
int main(int argc, char *argv[]) {
	...
	// Initialize
	std::cout << "Vector Addition cuMemMap with cuFile Pre-emptive Registration (Driver API)" << std::endl;
