 ...
 * 0.5x the size of the vectors and then doubles the size of the vectors using
 * cuMemMap-ed allocations. cuFile registers the initial device memory, then 
 * once the resize is complete, it re-registers the device memory and writes
 * the data to the files. It then proceeds to clear the device memory and	
 * read the data back from the files. Vector addition is then performed on
 * the GPU and the result is verified. Through the usage of cuMemMap-ed
 * allocations, the user can specify the physical properties of their memory
 * while retaining the contiguous nature of their access. Thus no change is
 * required to the program structure. 
 * 
 * This sample follows the same structure as the vectorAddDrv.cpp on 
 * CUDA samples repository.
 *
 * ./memmap_reregister <testfileA> <testfileB> 
 * 
 * | Output |
 ...
 * h_A(0xf3f16e200010), h_B(0xf3f160400010), h_C(0xf3f132200010), elements: 57671680
 * ...
 * Result = PASS
 * Cleaning up
 * Freeing d_A
...
int main(int argc, char *argv[]) {
	...
	// Initialize
	std::cout << "Vector Addition cuMemMap with cuFile (Driver API)" << std::endl;
	...
	std::cout << "Registering device memory for vector A of size: " << allocationSizeA << std::endl;
	status = cuFileBufRegister(reinterpret_cast<void*>(d_A), allocationSizeA, 0);
	if (status.err != CU_FILE_SUCCESS) {
	...
	std::cout << "Registering device memory for vector B of size: " << allocationSizeB << std::endl;
	status = cuFileBufRegister(reinterpret_cast<void*>(d_B), allocationSizeB, 0);
	if (status.err != CU_FILE_SUCCESS) {
	...
	check_cudadrivercall(simpleMallocMultiDeviceMmapResize(d_A, va_size, &allocationSizeA, size, backingDevices, mappingDevices));
	...
	// Re-register device memory A
	std::cout << "Re-registering device memory d_A(" << std::hex 
		<< d_A << ") of size: " << std::dec << allocationSizeA << std::endl;
	status = cuFileBufRegister(reinterpret_cast<void*>(d_A), allocationSizeA, 0);
	if (status.err != CU_FILE_SUCCESS) {
		std::cerr << "buffer register A failed: "
			<< cuFileGetErrorString(status) << std::endl;
	...
	// Re-register device memory B
	std::cout << "Re-registering device memory d_B(" << std::hex 
		<< d_B << ") of size: " << std::dec << allocationSizeB << std::endl;
	status = cuFileBufRegister(reinterpret_cast<void*>(d_B), allocationSizeB, 0);
	if (status.err != CU_FILE_SUCCESS) {
		std::cerr << "buffer register B failed:"
			<< cuFileGetErrorString(status) << std::endl;
	...
	// Write device memory A to file A
	std::cout << "Writing from device memory A to file: " << testfileA << std::endl;
	ret = cuFileWrite(cf_handle_A, reinterpret_cast<void*>(d_A), allocationSizeA, 0, 0);
	if (ret < 0) {
		...
		std::cout << "Written " << ret << " bytes to " << testfileA << std::endl;
		ret = EXIT_SUCCESS;
	}
