 ...
 * the Batch API's using half registered and half non-registered buffers.
 * For verification, input data has a pattern that increments by 1 for each
 * batch entry.
 *
 ...
 * ...
 * 
 * ./batch_write_unaligned <testfile> <gpuid> <numbatchentries(1-128)> <nondirectflag(0-all_direct, 1-half_direct)>
 * NOTE: nonDirFlag is optional, if not provided, it defaults to 0.
 *
 * | Output |
 * Opening file: <testfile>
 * Registering half of the device memory of size: 6000 for <numbatchentries> batch entries
 * Setting Up Batch
 * Submitting Batch IO
 * Submitting IO: index 0, size: 6000, file_offset: 0, devPtr: 0xf000b9e00000
 * ...
 * Batch IO Submitted
 * Got events: <(1-numbatchentries)>
 * Completed IO: index 0, size: 6000, file_offset: 0, devPtr: 0xf000b9e00000
 * ...
...
// Buffer size of 6000 bytes is not a multiple of disk's block size
static constexpr size_t MAX_BUFFER_SIZE_UNALIGNED = 6000;
...
int main(int argc, char *argv[]) {
	...
	for (size_t i = 0; i < batch_size; i++) {
		check_cudaruntimecall(cudaMalloc(&devPtrs[i], MAX_BUFFER_SIZE_UNALIGNED));

		// Fill device memory with pattern (specific per batch entry)
		check_cudaruntimecall(cudaMemset(static_cast<void*>(devPtrs[i]), 0xef + i, MAX_BUFFER_SIZE_UNALIGNED));
		check_cudaruntimecall(cudaStreamSynchronize(0));
	}

	// Registers device memory, only even indexed entries are using registered buffers. 
	std::cout << "Registering half of the device memory of size: " << MAX_BUFFER_SIZE_UNALIGNED << " for " << batch_size << " batch entries" << std::endl;
	for (size_t i = 0; i < batch_size; i++) {
		if ((i % 2) == 0) {
			status = cuFileBufRegister(devPtrs[i], MAX_BUFFER_SIZE_UNALIGNED, 0);
			if (status.err != CU_FILE_SUCCESS) {
				std::cerr << "Buffer register failed: "
					<< cuFileGetErrorString(status) << std::endl;
				registered_buffers = i;
				ret = EXIT_FAILURE;
				goto deregister_half_bufferhandles;
			}
		}
	}
	...
	for (size_t i = 0; i < batch_size; i++) {
		...
		io_batch_params[i].u.batch.file_offset = i * MAX_BUFFER_SIZE_UNALIGNED;
		io_batch_params[i].u.batch.devPtr_offset = 0;
		io_batch_params[i].u.batch.size = MAX_BUFFER_SIZE_UNALIGNED;
	...
	for (size_t i = 0; i < batch_size; i++) {
		std::cout << "Submitting IO: index " << i << ", size: " << MAX_BUFFER_SIZE_UNALIGNED 
			<< ", file_offset: " << i * MAX_BUFFER_SIZE_UNALIGNED << ", devPtr: " 
			<< devPtrs[i] << std::endl;
	}
...
// Cleanup labels
deregister_half_bufferhandles:
	for (size_t i = 0; i < registered_buffers; i++) {
		if ((i % 2) == 0) {
			CUfileError_t status = cuFileBufDeregister(devPtrs[i]);
			if (status.err != CU_FILE_SUCCESS) {
