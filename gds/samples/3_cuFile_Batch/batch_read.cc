 ...
 * This sample program reads data from a file to GPU memory
 * using the Batch API's. For verification, SHASUM is used 
 * on the file and the allocated device pointers per batch entry.
 * 
 * ./batch_read <testfile> <gpuid> <numbatchentries(1-128)> <nondirectflag(0-all_direct, 1-half_direct)>
 ...
 * ...
 * Batch IO Submitted
 * Got events: <numbatchentries>
 * Completed IO: index 0, size: 4096, file_offset: 0, devPtr: 0xebe2f9e00000
 * ...
...
int main(int argc, char *argv[]) {
	...
	// Fill the test file with random data using POSIX API
	if (!createTestFile(testfile, MAX_BUFFER_SIZE * batch_size)) {
		std::cerr << "Failed to create test file: " << testfile << std::endl;
		return EXIT_FAILURE;
	}
	...
	while (num_completed != batch_size) {
		...
		for (size_t i = 0; i < nr; i++) {
			...
			check_cudaruntimecall(cudaMemcpy(buf.data(), io_batch_params[i].u.batch.devPtr_base, io_batch_events[i].ret, cudaMemcpyDeviceToHost));
			std::cout << "Completed IO: index " << i << ", size: " << io_batch_params[i].u.batch.size
				<< ", file_offset: " << io_batch_params[i].u.batch.file_offset << ", devPtr: " 
				<< io_batch_params[i].u.batch.devPtr_base << std::endl;
		}
	}
	std::cout << "Batch IO Get status done got completions for " << nr << " events" << std::endl;
