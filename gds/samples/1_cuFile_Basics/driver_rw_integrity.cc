 ...
 * where CUDA Driver APIs are utilized.
 ...
 *
 * ./driver_rw_integrity <testWriteReadFile> <testWriteFile> <gpuid>
 *
 * | Output |
...
int main(int argc, char *argv[]) {
	...
	// CUDA & cuFile Specific Variables
	CUdevice cudev = 0;
	...
	check_cudadrivercall(cuDeviceGet(&cudev, parseInt(argv[3])));
	...
	ret = cuFileRead(cf_handle, reinterpret_cast<void*>(cudevPtr), SIZE, 0, 0);
	if (ret < 0) {
	...
	// Write read data from GPU memory to a new file
	...
	ret = cuFileWrite(cf_handle, reinterpret_cast<void*>(cudevPtr), SIZE, 0, 0);
	if (ret < 0) {
		if (IS_CUFILE_ERR(ret))
...
close_file:
	if (fd >= 0) close(fd);
cuda_cleanup:
	if (cudevPtr) check_cudadrivercall(cuMemFree(cudevPtr));
	if (cuCtx) check_cudadrivercall(cuCtxDestroy(cuCtx));
